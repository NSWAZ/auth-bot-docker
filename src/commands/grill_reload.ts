import { SlashCommandBuilder, ChannelType, ActionRowBuilder, ButtonBuilder, ButtonStyle, CollectorFilter, MessageComponentInteraction, TextChannel, VoiceChannel } from 'discord.js';
...
const GrillReloadCommand: SlashCommand = {
	...
	execute: interaction => {
		...
		const targetChannel = interaction.options.getChannel('targetchannel') as TextChannel | VoiceChannel;
		...
		void interaction.reply({ content: `정말 ${targetChannelName}을 아카이브하고 복사본을 만들까요?`, components: [row] })
			.then(async () => {
				const filter: CollectorFilter<[MessageComponentInteraction<'cached'>]> = i => i.customId === 'yes' || i.customId === 'no';

				if (interaction.channel === null) throw Error('명령어 입력 채널을 찾을 수 없습니다.');
				...
				if (collected.customId === 'yes') {
					if (interaction.guild === null) throw Error('명령어 입력 길드를 찾을 수 없습니다.');

					const targetCategories = interaction.guild.channels.cache.filter(c => c.type === ChannelType.GuildCategory && c.name === '아카이브');

					switch (targetCategories.size) {
					case 1:
						void targetChannel.clone()
							.then(() => {
